...
a_true+=abs(rng.normal(scale=1E-3, size=(n_MZI,)))
b_true=np.zeros((n_MZI,))
b_true+=abs(rng.normal(scale=1E-3, size=(n_MZI,)))
c_true=np.zeros((n_MZI,))
c_true+=abs(rng.normal(scale=1E-3, size=(n_MZI,)))

C_true=np.zeros((n_MZI,))
C_true+=200+rng.normal(scale=1, size=(n_MZI,))
...
def DataGen(MZI_no,input,output):
    ...
    for i in range(N):
        ...
        MatArray.append(modeU[input,output])
    return MatArray
...
def curvefit(MZI_no,InputPort,OutputPort):
    ...
    with pm.Model() as model:  # model specifications in PyMC are wrapped in a with-statement
        ...
        likelihood = pm.Normal("P_opt", mu=A*pm.math.cos(C * (a_true[MZI_no] * V + b_true[MZI_no] * (V ** 2) + c_true[MZI_no] * (V ** 3)) - theta_0) + B, sigma=.1, observed=clickdata)
...
for _ in range(len(MatArray_test_res)):
    #Fid.append(np.absolute(np.trace(MatArray_test_true[_].conj().T@MatArray_test_res[_])))
