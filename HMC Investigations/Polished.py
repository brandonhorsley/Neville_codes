...
def main():
    ...
    def construct_BS_pymc(RV):
        ...
        return pt.stack([pm.math.sqrt(RV), 1j*pm.math.sqrt(1-RV), 1j*pm.math.sqrt(1-RV), pm.math.sqrt(RV)])
    def construct_PS_pymc(RV):
        #return pt.stack([pm.math.exp(1j*RV/2),0,0,pm.math.exp(-1j*RV/2)]).reshape((2,2))
        return pt.stack([pm.math.exp(1j*RV/2),0,0,pm.math.exp(-1j*RV/2)])
    ...
    def circuit_list_to_matrix_pymc(feature):
        ...
        index=feature[2]-1
        if feature[0]=="BS":
            matrix=pt.eye(n=m,m=m,dtype="complex128")
            matrix=pt.set_subtensor(pt.flatten(matrix)[0:2],construct_BS_pymc(feature[1])[0:2])
            matrix=pt.set_subtensor(pt.flatten(matrix)[2:4],construct_BS_pymc(feature[1])[2:4])
            matrix=matrix.reshape((2,2))
            matrix=[matrix]*N #To copy it N times to help matrix dotting across to get final U
        if feature[0]=="PS":
            matrix=[]
            for _ in range(N):
                blank=pt.eye(n=m,m=m,dtype="complex128")
                blank=pt.set_subtensor(pt.flatten(blank)[0:2],construct_PS_pymc(feature[1][0][_])[0:2])
                blank=pt.set_subtensor(pt.flatten(blank)[2:4],construct_PS_pymc(feature[1][0][_])[2:4])
                blank=blank.reshape((2,2))
                matrix.append(blank)
    ...
    def circuit_list_to_matrix_pymc_2(feature):
        #Had to be quirky in construct_BS_pymc and construct_PS_pymc since set_subtensor doesn't like replacing 2D tensors or rows or even element by element (breaks down after one element) so had to return a flat tensor and reassign half of the array and then the other half.
        index=feature[2]-1
        if feature[0]=="BS":
            real=pt.eye(n=m,m=m)
    ...
    with pm.Model():
        ...
        U_list = np.array([circuit_list_to_matrix_pymc_2(feature) for feature in circuit_list])
        ...
        for i in range(N):
            #Dot product code since matrix_dot seems to bug out, see if you can duplicate it and report.
            rval = U_list[:,i][0]
            for a in U_list[:,i][1:]:
                #rval = pt.dot(rval, a)
